...
async def analyze(file: UploadFile = File(...)):
    ...
    try:
        ...
        # 1. Duplicate check
        file_hash = calculate_file_hash(path)
        existing_docs = db.collection('cases').where(filter=FieldFilter('file_hash', '==', file_hash)).stream()
        ...
        # 2. AI Analysis & Outcome Prediction
        report = analyze_case_with_gemini(path)
        prediction = predict_outcome(report.get('summary', ''))

        # 3. Vector Embedding
        vector = get_embedding(report.get('summary', ''))
