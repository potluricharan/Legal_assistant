...
def calculate_file_hash(path):
    """Generates an MD5 hash fingerprint for duplicate detection."""
    hasher = hashlib.md5()
    try:
        with open(path, 'rb') as f:
            for chunk in iter(lambda: f.read(4096), b""):
...
def get_embedding(text):
    """Generates vector embeddings with safe fallback."""
    try:
...
def analyze_case_with_gemini(pdf_path):
    """Extracts legal summary and analysis using Gemini."""
    text = extract_text_from_pdf(pdf_path)
    if not text:
        return {"summary": "No readable text found in the uploaded PDF document."}

    prompt = (
        "You are a Senior Legal Analyst. Analyze this court document and provide:\n"
        "1. A concise executive summary.\n"
        "2. Key legal arguments presented.\n"
        "3. Potential procedural or investigation gaps found in the case.\n\n"
        f"TEXT CONTENT:\n{text[:10000]}"
    )
...
def analyze_with_history(case_id, user_query):
    """Context-aware chatbot using Firestore history."""
    try:
        case_ref = db.collection('cases').document(case_id)
        case_doc = case_ref.get()
        ...
        history_ref = case_ref.collection('case_history').order_by('timestamp', direction='DESCENDING').limit(5).stream()
        history_list = list(history_ref)
        history_text = "\n".join([doc.to_dict().get('content', '') for doc in reversed(history_list)])
        ...
        full_prompt = (
            ...
            f"Recent Chat History: {history_text}\n"
            f"User Query: {user_query}\n\n"
            f"Instructions: Answer the query professionally and maintain continuity."
        )
